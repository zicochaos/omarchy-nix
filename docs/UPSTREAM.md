...
These are the defaults encoded in the upstream source (rev `35212998`,
...
| **Deferred** | Possible on NixOS, not done | NordVPN service (menu entry deleted; the nixpkgs package + `services.nordvpn` land only in the 26.11 cycle; re-add as a catalog feature when the stable pin catches up); zen / brave-origin browsers (AUR-only, no nixpkgs attrs on the 26.05 pin; menu entries deleted) |
